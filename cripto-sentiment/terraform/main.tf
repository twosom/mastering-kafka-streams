...
resource "null_resource" "create_topic" {
  ...
  provisioner "local-exec" {
    command = "docker exec -i kafka-1 kafka-topics --bootstrap-server localhost:9092 --topic tweets --replication-factor ${local.kafka_count} --create"
  }
}
